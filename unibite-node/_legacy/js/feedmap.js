document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadFeed() {
        ...
        fetch('/unibite/api/feed/list.php')
            .then(response => response.json())
            .then(data => {
                if (data.success) {
    ...
    function loadNearby() {
        ...
        const url = `/unibite/api/feed/nearby.php?lat=${refLat}&lng=${refLng}`
            + `&max_km=${lastNearby.maxKm}&limit=${lastNearby.limit}`;
    ...
    function renderListings(listings) {
        ...
        listings.forEach(listing => {
            ...
            const photoSrc = listing.photo_filename
                ? `/unibite/uploads/${listing.photo_filename}`
    ...
    listingsGrid.addEventListener('click', (e) => {
        ...
        fetch('/unibite/api/requests/send.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ listing_id: listingId, slot: slot })
        })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
